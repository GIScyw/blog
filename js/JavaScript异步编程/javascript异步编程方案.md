...
![img](https://user-gold-cdn.xitu.io/2019/1/5/1681d3f072df2e14?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2019/1/5/1681d42b6e8708fc?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
- **-async函数返回一个Promise，一个函数如果加上 async ，那么该函数就会返回一个 Promise，async就是将函数返回值使用Promise.resolve()包裹了下**
...
- **await关键词可以暂停async函数的执行，直到它受到了一个promise的结果。await 后面可以跟任何的JS 表达式。虽然说 await 可以等很多类型的东西，**但是它最主要的意图是用来等待 Promise 对象的状态被 resolved**。如果await的是 promise对象会**造成异步函数停止执行并且等待 promise 的解决**,如果等的是正常的表达式则立即执行。**
...
- **async特别的要注意它对错误的处理。一定要用try catch来处理错误**
